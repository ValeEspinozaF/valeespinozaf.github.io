...
function bibtex2html($entry, $type, $accents, $hightlightName = '', $authorLimit = 0){
	...
	if(trim($ret)==""){
		...
		if ($author == "") {
			$author = extractBib("lector", $entry, $accents);
		}
		...
		switch($type) {
			case "workshop":
				$month		= extractBib("month", $entry, $accents); 
				$city 		= extractBib("city", $entry, $accents);
				$country 	= extractBib("country", $entry, $accents);
				$conference	= extractBib("conference", $entry, $accents);
				if($conference == "") {
					$conference = extractBib("institution", $entry, $accents);
				} else {
					$conference = ' <span class="conference">'.$conference.'</span>';
				}

				// Build output
				$ret .= $year;
				$ret .= ', ' . $month . ':';
				if(trim($conference)			!= "") $ret .= ' ' . $conference . '. ';
				$ret .= '<span class="oral_title">' . $title . '</span>. ';
				$ret .= '<span class="authors">'.formatAuthors($author, $hightlightName, $authorLimit).'</span> ';	
				
				if(trim($city)			!= "") $ret .= $city;
				if(trim($country)		!= "") $ret .= ', <span class="country">'.$country.'</span>';
				$ret = $ret.".";				
				break;
			case "poster":
			case "oral":
			...
			case "software":
				$ret = $ret . '<span class="authors">'.formatAuthors($author, $hightlightName, $authorLimit).'</span> ';
				$version = extractBib("version", $entry, $accents);
				$ret .= '<i>'.$title.'</i>. ';
				
				// Build output
				$ret = $ret." software";
				if(trim($version)	!= "") $ret = $ret.' '.$version;
				if(trim($year)		!= "") $ret = $ret.', '.$year;
				$ret = $ret.".";
				break;
		...
		if(!$doilinked && trim($doi) != "") {
			$ret .= ' <span class="doi"><a href="'.$doi.'" target="_blank">{% octicon link-external height:16 %}</a></span>&nbsp;';
		}
 ...
 */
function bibfile2html($filename, $displayTypes = NULL, $groupType = NULL, $groupYear = NULL, $groupDate = NULL, $bibLink = NULL, $highlightName = NULL, $numbersDesc = NULL, $sorting = NULL, $authorLimit = NULL, $abstractLink = NULL) {
	return bibstring2html(file($filename), $displayTypes, $groupType, $groupYear, $groupDate, $bibLink, $highlightName, $numbersDesc, $sorting, $authorLimit, $abstractLink);
}
 ...
 * @author Andreas Classen
 */
function bibstring2html($fileContent, $displayTypes = NULL, $groupType = NULL, $groupYear = NULL, $groupDate = NULL, $bibLink = NULL, $highlightName = NULL, $numbersDesc = NULL, $sorting = NULL, $authorLimit = NULL, $abstractLink = NULL) {
	...
	if($groupDate === null) $groupDate = true;
	...
	$dateTypes = array("Upcoming" => "Upcoming", "Passed" => "Passed");
	...
	for($i = 0; $i < $len; $i++) {
		if(substr($fileContent[$i], 0, 1) == '@') {
			...
			$month = extractBib("month", $bibEntry, $accentTable);
			...
			if($sorting == null) $index = sprintf("%08d", $j);
				...
				foreach($sorting as $field) {
					...
					else if($field == 'month') $index .= sprintf("%02d", 12 - (int) date("m", strtotime(extractBib("month", $bibEntry, $accentTable))));
					...
					} else if($field == 'lector') {
						$temp = html_entity_decode(extractBib("lector", $bibEntry, $accentTable));
						$index .= formatAuthors($temp, "", $authorLimit);
					} else {
			...
			$entryDate = DateTime::createFromFormat('Y M d', $year . ' ' . $month . ' 01');
			...
			if($year != '' && $text != '') {
				...
				elseif($groupType && $groupDate) $entries[$date][$type][$index] = $element;
